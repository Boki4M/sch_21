...
int count_live_neighbors(char field[HEIGHT][WIDTH], int row, int col);  // подчет живых клеток
void update_field(char current[HEIGHT][WIDTH],
                  char next[HEIGHT][WIDTH]);             // обновление игрового поля относительно правил игры
void print_field(char field[HEIGHT][WIDTH], int delay);  // вывод игровова поля
void initialize_field(char field[HEIGHT][WIDTH]);        // инициализация стартового значения поля
...
int main() {
    ...
    // два массива содержащие
    char current[HEIGHT][WIDTH] = {0};
    char next[HEIGHT][WIDTH] = {0};

    initialize_field(current);
    initialize_field(next);

    int delay = 300000;
    const int min_delay = 50000;
...
void print_field(char field[HEIGHT][WIDTH], int delay) {
    ...
    int fps = (delay > 0) ? 1000000 / delay : 0;
    mvprintw(HEIGHT, 0, "A (faster), Z (slower), Spacebar (exit)");
    mvprintw(HEIGHT + 1, 0, "Speed: %d mks (%d fps)", delay, fps);
